...
def Mymeasure(y_h0_x, tag_label):
    ...
    yh0_PD_avg = np.mean(yh0_PD, axis=(0,))
    yh0_AC_avg = np.mean(yh0_AC, axis=(0,))
    yh0_GD_avg = np.mean(yh0_GD, axis=(0,))
...
def train():
    ...
    with open('./results/' + name_of_data +str(ii)+'.txt', "a+") as f:
        f.write(results_txt)
    with open('./results/' + "pre_label" +str(ii)+ '.txt', "a+") as f:
